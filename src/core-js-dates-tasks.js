...
function getTime(date) {
  const hours = date.getHours() / 100;
  const minutes = date.getMinutes() / 100;
  const seconds = date.getSeconds() / 100;
  return [
    hours.toFixed(2).slice(2),
    minutes.toFixed(2).slice(2),
    seconds.toFixed(2).slice(2),
  ].join(':');
}
...
function getCountDaysInMonth(month, year) {
  const date1 = new Date(year, month - 1, 1 + 1);
...
function isDateInPeriod(date, period) {
  let flag = false;
  const dateMiddle = new Date(date).getTime();
  const dateStart = new Date(period.start).getTime();
  const dateEnd = new Date(period.end).getTime();
  if (dateMiddle >= dateStart && dateMiddle <= dateEnd) flag = true;
  return flag;
}
...
function getCountWeekendsInMonth(month, year) {
  const startDay = new Date(year, month - 1, 1, 15);
  let count = 0;
  while (startDay.getMonth() + 1 === month) {
    if (startDay.getDay() === 0 || startDay.getDay() === 6) count += 1;
...
function getWeekNumberByDate(date) {
  ...
  while (date.getFullYear() === year) {
    if (date.getFullYear() === year) count += 1;
    date.setDate(date.getDate() - 7);
  }
  date.setDate(date.getDate() + 7);
  if (date.getDay() < date.getDate() && date.getDay() !== 0) count += 1;
  return count;
}
...
function getNextFridayThe13th(date) {
  while (true) {
    ...
    if (date.getDate() === 13 && date.getDay() === 5) break;
  }
  return date;
}
...
function getQuarter(date) {
  let result;
  const obj = { 1: [0, 1, 2], 2: [3, 4, 5], 3: [6, 7, 8], 4: [9, 10, 11] };
  for (let i = 0; i < 4; i += 1) {
    const key = Object.keys(obj)[i];
    if (obj[key].indexOf(date.getMonth()) > -1) result = key;
  }
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (day <= endDate) {
    for (let i = 0; i < countWorkDays; i += 1) {
      const newDate = `${(day.getDate() / 100).toFixed(2).slice(2)}-${((day.getMonth() + 1) / 100).toFixed(2).slice(2)}-${day.getFullYear()}`;
      if (day <= endDate) {
        result.push(newDate);
        day.setDate(day.getDate() + 1);
      }
    }
    day.setDate(day.getDate() + countOffDays);
  }
  return result;
}
